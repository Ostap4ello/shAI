...
This is an AI-powered code agent with focus on software development tasks and
terminal integration. It supports various tools and is context- and
project-aware. It is able to use both remote and local LLMs. It is also
open-source. \cite{aider-web} \cite{aider-docs}.\\

\subsubsection{termai}

This is a terminal-based AI assistant similar to gptme, written in Rust. It
supports various LLM providers, but it is not able to use local LLMs in its
...
I/O. It sends the terminal output as context to chosen LLM and gets the
response back. It has execution agent capabilities, which allows it to run
commands in terminal based on LLM response. It is open-source and is adapted to
use with OpenAI API, thus and it can be used with local LLMs as well.\\

We can inspire from its approach to bash integration using tmux sessions to
provide seamless interaction between LLM and terminal. Though, this approach
turned out to be somewhat clunky in practice, as it couldn't handle well the
TUI applications, but only get bash's responses. \cite{aiterm-web} \\


\subsubsection{Other solutions}

There are also such solutions, like \textsl{ai-terminal-assistent (github)}{}
or \textsl{cmd-ai (github)}, which are just wrappers for Rest API calls to
remote LLMs. They provide basic AI capabilities in terminal, but have no significant
features.
