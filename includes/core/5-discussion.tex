...
\paragraph{Agent-Based Approaches --- Unreliable In Current State}
Agentic systems that attempt multi-step reasoning, tool use, and state
management  have shown to consistently fail with small local \glspl{llm} on
stage of early development and testing. In current state, the approach is totaly
...
retrieved documents, the system achieves: reliable accuracy (ranging from 75\%
to 94\% Top-5 accuracy depending on the approach), acceptable retrieval latency (under 3s
...
for the light configuration and 39.78--56.57s for the heavy configuration), due
to huge generated output size.
