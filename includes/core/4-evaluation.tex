...
For end-to-end testing, we reuse the \texttt{tests/end-to-end/test\_cases.json}
query set (10 queries) and run it against the \texttt{sample-10},
\texttt{sample-100}, and \texttt{sample-1000} corpora.
...
\begin{table}[h]
  ...
  \scriptsize
    ...
    \midrule
    sample-10   & 1.59 & 2.22 & 1.61 \\
    sample-100  & 1.58 & 2.03 & 1.58 \\
    sample-1000 & 1.58 & 1.96 & 1.47 \\
    \bottomrule
...
\begin{table}[h]
  ...
  \scriptsize
    ...
    \textbf{Document-level with passage-level re-ranking} & \textbf{Passage-level} \\
    \midrule
    sample-10   & 1.60 & 6.92 & 1.60 \\
    sample-100  & 1.59 & 8.18 & 1.58 \\
    sample-1000 & None & None & None \\
    \bottomrule
...
\begin{table}[h]
  ...
  \scriptsize
    ...
    \textbf{Document-level with passage-level re-ranking} & \textbf{Passage-level} \\
    \midrule
    sample-10   & 17.21 & 6.38 & 6.47 \\
    sample-100  & 12.39 & 7.47 & 5.72 \\
    sample-1000 & None  & None & None \\
    \bottomrule
...
\begin{table}[h]
  ...
  \scriptsize
    ...
    \textbf{Document-level with passage-level re-ranking} & \textbf{Passage-level} \\
    \midrule
    sample-10   & 39.78 & 9.57 & 8.66 \\
    sample-100  & 56.57 & 10.77 & 8.86 \\
    sample-1000 & None & None & None \\
    \bottomrule
...
\paragraph{Key Findings}
Document-level retrieval yields the lowest search latency (1.58--1.60s for
Granite; 1.59--1.60s for Qwen3 embeddings), but produces the highest RAG latency
due to large context sizes (12.39--17.21s for the light configuration and
39.78--56.57s for the heavy configuration). Document-level retrieval with
passage-level re-ranking increases search latency (2.03--2.22s for Granite;
6.92--8.18s for Qwen3 embeddings) while reducing RAG latency to 5.69--7.47s
(light) and 9.57--10.77s (heavy). Passage-level retrieval keeps search latency
close to document-level values (1.47--1.61s and 1.58--1.60s) while providing the
lowest RAG latency overall (5.34--6.47s light; 8.66--8.86s heavy).
