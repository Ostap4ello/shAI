...
execute commands and other \gls{cli} utilities~\cite{linfo_terminal}.
...
In the original article, previously mentioned transformers consist of two main
parts~--- \textbf{Encoder} and \textbf{Decoder}. The usage of the architecture
based on attention implies that the model generates the output text
sequentially, one token at a time, while attending to all previously generated
tokens. This can be a limitation for long prompts, as the model can only attend
to a limited number of previous tokens (this limit is called \textbf{context
length} or \textbf{window size}, and it is described more in detail in
Paragraph~\ref{p:context-window}).

\paragraph{Encoder} The encoder maps an input sequence of symbols (tokens) to a
...
\paragraph{Decoder} The decoder generates an output sequence of symbols
...
\paragraph{Context Window}\label{p:context-window} The context window is the
maximum number of tokens that a Transformer-based model can process at once,
meaning that only the tokens within the context window can affect the next token
prediction. As mentioned before, the smaller is the prompt, the faster is the
processing and the more impact has each individual token, as all empty tokens
are practically ignored. In contrast, the larger is the prompt, the slower is
the processing and the less impact has each individual token, meaning that some
information can be lost. In case of a prompt larger than the context window, the
tokens that exceed the context window are ignored, and the information is
definitely lost.
...
than dense models within the same computational budget~\cite{dai2024deepseekmoe}.
...
pseudocode in Algorithm~\ref{alg:agent}.
...
sources. We define the workflow of \gls{rag} in Algorithm~\ref{alg:rag}.
...
\STATE \textbf{function} \textsc{LLM\_RAG\_generate}(\textit{user\_query}, \textit{LLM}, \textit{document\_database})
    ...
    \STATE \quad \COMMENT{Step 3: Pass the augmented prompt to the LLM and
    return it as the final response}
    \STATE \quad \textbf{return} $\gets \textit{LLM}.\text{generate\_response}(\textit{augmented\_prompt})$
\STATE \textbf{end function}
\end{algorithmic}
\end{algorithm}
...
be fit into \gls{llm} context window (see Paragraph~\ref{p:context-window}) or
answer size.
...
for testing and validation and by the term \textbf{document} we reffer to a man
