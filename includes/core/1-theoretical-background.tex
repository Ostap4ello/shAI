...
\STATE \textbf{function} \textsc{LLM\_Agent}(\textit{task}, \textit{context})
    ...
    \STATE \quad $\textit{final\_output} \gets \textit{LLM}.\text{summarize\_results}(\textit{results})$
    \STATE \quad \textbf{return} $\textit{final\_output}$
...
agents~\cite{anthropic2024mcp}. While not used in this thesis, it is important
to mention it as a relevant concept~\cite{zhao2023}.
...
in Appendix~\ref{a:man}. A manual page consists of several sections.
Conventional section names are listed in Appendix~\ref{a:man}. The most
