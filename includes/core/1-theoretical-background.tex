...
trained on massive text data, such as \gls{gpt}, \gls{bert}, \gls{lstm} or
Mamba.\@ \glspl{llm} exhibit strong capacities to understand natural language
and solve complex tasks (via text generation)
\cite{radford2018improving,devlin2018bert,touvron2023llama}.\\
...
semantic meaning of the text and can be used in various applications.
\cite{nie2025textembeddingmeetslarge}.\\
...
tokens more effectively than traditional embedding models
\cite{zhang2024systematicsurveytextsummarization,khan2026retrieval}.\\
...
agents\cite{mcp}. While not used in this thesis, it is important to mention it
as a relevant concept\cite{zhao2023}.
...
We can define the workflow of \gls{rag} as follows as pseudocode in Listing\
\ref{lst:rag}.
...
is to use \textbf{semantic document search and retrieval}\ref{emb:semvec}. By
this, we mean the set of techniques of indexing and searching documents based on
their semantic representations. Therefore, there are two main components for the
...
performance on the target task while keeping low training effort and costs.
\cite{zhao2023}.
