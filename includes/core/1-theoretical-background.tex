...
\begin{itemize}
    ...
    \item \textbf{Transformer Layers}: These layers are the core of the LLM architecture,
        responsible for processing

Currently, LLMs are mainly built upon the Transformer architecture
\cite{attention}, where \textbf{multi-head attention layers} are stacked in a
very deep neural network. They are based on the Transformer architecture.\\
...
When user submits a query, the system retrieves relevant documents from
document database. These documents are then injected to the prompt sent to the
LLM, providing it with additional context to generate a more accurate response.
...
\textbf{Semantic representation} of documents, with introduction of LLMs, have
...
\begin{itemize}
    \item \textbf{Sparse vector representations (Sparse retrieval):} This
        ...
        the dictionary of all possible words (or subset of them) to be defined.
...
% describe apis for openai which will be used?
% mcp?
