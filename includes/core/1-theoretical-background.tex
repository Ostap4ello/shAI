...
solution. It defines two interfaces: Command Line Interfaces (CLIs) and
Text-based User Interfaces (TUIs). It also describes the programs that leverage
these interfaces: shells, CLI utilities, and TUI applications.

For the beginning, we need to understand what Command Line Interfaces (CLIs)
are, as well as the utilities that are used within them.\\
...
\begin{quote}
  ...
  devices, humans, and combinations of these.}}
\end{quote}\cite{hookway2014}\\

\subsubsection{Terminal, Terminal emulator. Terminal Application}

The article focuses on text-based \textbf{terminal} interfaces, which are a
specific type of interface used in computing environments. Here is how we
define terminals, terminal emulators and terminal applications.\\
...
data from, a computer or a computing system}}. This definition
defines main functionality of terminals and terminal emulators, which
is to provide an elementary, which is
CLI, interface for user to interact with the computer system.\\
...
--- either CLIs or TUIs. We will discuss it further in the following
sections.\\
...
The definition of CLI extends previous definition to the specific case of
...
\textbf{Shells} are special programs that provide relatively wide and direct
access on system on which it runs. Most shells are CLI programs. They interpret
user commands, execute them, and return the results.

\textbf{CLI utilities} are programs that are executed from within the CLI
environments.\\
...
Text-based User Interfaces (TUIs) are a type of user interface that is a
further extension of CLIs. They provide a more visually structured and
...
This section provides a brief overview of the aspects of Large Language Models
(LLMs), relevant for this article. It describes the architecture, types,
limitations of LLM, the main protocols for the interaction, as well as the
advanced concepts, like extension and fine-tuning.
...
data, such as GPT, BERT or LLaMA.\@ LLMs exhibit strong capacities to understand
natural language and solve complex tasks (via text generation).

The architecture of LLMs can be devided into several main components:
\begin{itemize}
  ...
  \item \textbf{Embedding Layer}: This layer converts the tokenized input
    ...
    and \textbf{contextual}. Static embeddings, such as Word2Vec or GloVe,
    assign a fixed vector to each token, regardless of the context in which
    it appears. Contextual embeddings, on the other hand, generate
    different vectors for the same token based on its context in the input
    text, allowing for a more nuanced understanding of language.
  \item \textbf{Transformer Layers}: These layers are the core of the LLM
...
As mentioned above, LLMs are mainly built upon the Transformer
...
The transformer architecture is based on finding relationships between elements
in a sequence using self-attention mechanisms\cite{naveed2023}. This enables
the model to recognize patterns and dependencies in the input, which may be
helpful for decrypting ciphers that rely on patterns in the text.
...
the output, as the model has to process all previous tokens for each new token.
...
Encoder-Decoder models. % TODO: citation from survey zhao2023
...
\subsubsection{AI Agent Systems}\label{agent}
...
\begin{lstlisting}[,
  caption={Pseudocode for LLM Agent workflow},
  label={lst:agent},
  language=pseudo,
  style=code-listing
]

function LLM\_Agent(task, context):
    subtasks = LLM.decompose\_task(task)  # Break down the main task into smaller sub-tasks
    for subtask in subtasks:
        switch subtask.type:
            case "tool":
                result = use\_tool(subtask.tool, subtask.tool\_input)
            case "further\_decomposition":
                result = AI\_Agent(subtask.task, context)  # Recursive call for further decomposition
            case "generate":
                result = LLM.generate\_response(subtask.task)
        results.append(result)
    final\_output = LLM.summarize\_results(results)
...
\begin{lstlisting}[,
  caption={Pseudocode for Retrieval-Augmented Generation (RAG)},
  label={lst:rag},
  language=pseudo,
  style=code-listing
]

function LLM\_RAG\_generate(user\_query, LLM, document\_database):
    # Step 1: Retrieve relevant documents from the document database
    relevant\_documents = retrieve\_documents(document\_database, user\_query)

    # Step 2: Inject retrieved documents into the input prompt for the LLM
    augmented\_prompt = construct\_prompt(user\_query, relevant\_documents)

    # Step 3: Pass the augmented prompt to the LLM to generate a response
    llm\_response = LLM.generate\_response(augmented\_prompt)
...
This case involves document retrieval, for which it is the best to use the
\textbf{semantic document search and retrieval}. By this, we mean the
techniques of indexing and searching documents based on their semantic
representations. Therefore, there are two main components for the retrieval
system: document representation generator and storage / index with searching
techinques best suited for the task.\\

\textbf{Semantic representation} of documents, with introduction
of LLMs, have
divided into two major groups:
\begin{itemize}
  \item \textbf{Sparse vector representations (Sparse retrieval):} This
    method mostly relies on traditional keyword-based approaches, such as
    BoW, one-hot encoding, LSI or TF-IDF to retrieve documents based on the
    presence or quantity of specific terms. It is computationally efficient
    and works well for simple queries, but it may struggle with complex
    queries or those that require understanding of context and semantics.\\
    \textsl{The common example of sparse retrieval is Bag-of-Words (BoW)
      model, which represents documents as vectors of word counts. It needs
      the dictionary of all possible words (or subset of them) to
      be defined.
      Then, the size of the vector is equal to the size of the dictionary,
    and each element position corresponds to the word in the dictionary.}
  \item \textbf{Dense vector representations (Dense retrieval):} This method
    uses dense vector representations (embeddings) of documents and queries
...
\textbf{Document representation/summarization vector generator techniques}
is also an important aspect of the representation of documents. As
embedding layer of either dense or sparse retrieval represents the document
as a collection of vectors, one for each token, it is important to have the
techniques to generate the single vector that represents this whole
collection.
\begin{itemize}
  \item \textbf{Classical Approaches:} Theese techniques include
    ...
    frequency of words or presence and relationships between
    based on it, but not the context.
  \item \textbf{Neural Network-based Approaches:} These
    techniques involve training a neural network, such as a
    transformer or recurrent neural network, to generate a single
    vector representation for the entire document. The possible
    approaches are:
    \begin{itemize}
      ...
      \item \textbf{[CLS] token-based summarization:} This method uses a
        special token (e.g., [CLS] in BERT) that is added to the
        ...
        summary vector for the document.
    \end{itemize}
...
manage high-dimensional vector data (in this case, document embeddings) and
perform efficient similarity search based on different metrics (e.g., cosine
similarity, L2 or Euclidean distance). Examples of vector databases include
Faiss, Annoy, and Milvus.
