...
Modely typu \gls{moe} sú výpočtovo efektívnejšie, ale dosahujú horší výkon ako rovnako veľké dense modely s~rovnakým celkovým počtom parametrov. Avšak v~prípadoch, keď nie je obmedzená pamäť VRAM, môže \gls{moe} prekonať výkon dense modelov pri zachovaní ekvivalentných výpočtových nákladov~\cite{dai2024deepseekmoe}.
...
\begin{itemize}
    ...
    \item \textbf{Husté (dense) vektorové reprezentácie} využívajú embedding \gls{llm} na generovanie embedding vektorov, ktoré zachytávajú sémantický význam textu. Tento prístup je efektívnejší pre zložité dopyty, vyžaduje si však viac výpočtových zdrojov než riedke prístupy~\cite{khan2026retrieval}.
...
Okrem core komponentov zahŕňa implementácia aj používateľské procesy pre inicializáciu systému, buildovanie indexu, vykonávanie dopytov a~zmeny konfigurácie.
