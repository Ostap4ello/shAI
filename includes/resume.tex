...
\paragraph{\gls{cli} nástroj} (utility) je program spúšťaný v prostrediach \gls{cli}.

\paragraph{\gls{tui}} rozširuje \gls{cli} tým, že poskytuje štruktúrované, intuitívne rozhrania s využitím knižníc ako ncurses a schopností terminálu~\cite{linfo_terminal}.

\subsection*{Koncepty \gls{llm}}

\paragraph{Jazykové modely} predpovedajú a generujú textové sekvencie, čo je kľúčové pre preklad, sumarizáciu a tvorbu obsahu. Ich vývoj napreduje spolu s pokrokmi v oblasti neutrónových sietí~\cite{naveed2023}.

\paragraph{\gls{llm}} sú jazykové modely založené na architektúre transformer s veľkým počtom parametrov, trénované na masívnych textových datasetoch (napr. \gls{gpt}~\cite{radford2018improving}, \gls{bert}~\cite{devlin2018bert}, \gls{lstm}~\cite{hochreiter1997lstm} alebo Mamba~\cite{gu2023mamba}). Vykazujú silné porozumenie prirodzenému jazyku a schopnosť riešiť komplexné úlohy prostredníctvom generovania textu.
...
\begin{itemize}
    ...
    \item \textbf{Embeddingová vrstva} prevádza tokenizovaný vstup na husté vektorové reprezentácie (ako sú fixné vektory alebo vektory závislé od pozície).

    \item \textbf{Transformerové vrstvy} pozostávajú z viacerých hláv mechanizmu pozornosti (attention)~\cite{attention} a dopredných (feedforward) sietí, ktoré modelu umožňujú zachytiť komplexné vzťahy medzi tokenmi.
...
\begin{itemize}
  \item \textbf{Enkóder} mapuje vstupné sekvencie tokenov na spojité reprezentácie pomocou multi-head self-attention a dopredných sietí.
  \item \textbf{Dekóder} autoregresívne generuje výstupné sekvencie tokenov. Zahŕňa subvrstvy v štýle enkódera a navyše multi-head attention nad výstupom enkódera.
\end{itemize}

Moderné modely \gls{llm} využívajú primárne \textbf{Decoder-only} architektúru, ktorá sa spravidla ľahšie škáluje, trénuje a jemne ladí (fine-tune) než modely typu Encoder-Decoder, pričom dosahuje lepší výkon pri rovnakom počte tréningových tokenov~\cite{zhao2023,wei2023chainofthoughtpromptingelicitsreasoning}.
...
Modely typu \gls{moe} sú výpočtovo efektívnejšie, ale dosahujú horší výkon ako rovnako veľké husté (dense) modely s rovnakým celkovým počtom parametrov. Avšak v prípadoch, keď nie je obmedzená pamäť VRAM, môže \gls{moe} prekonať výkon hustých modelov pri zachovaní ekvivalentných výpočtových nákladov~\cite{dai2024deepseekmoe}.

\subsubsection*{Embeddingové modely \gls{llm}}

\paragraph{Embeddingové \gls{llm}} sú neurónové modely založené na architektúre transformer, ktoré slúžia na generovanie vektorových reprezentácií dokumentov.
...
\begin{itemize}
  ...
  \item \textbf{Používanie nástrojov:} Využívajú externé nástroje, RAG pipelines alebo API nad rámec samotného generovania textu.
...
\subsubsection*{Reprezentácia dokumentov}
...
\begin{itemize}
    \item \textbf{Riedke (sparse) vektorové reprezentácie} využívajú tradičné prístupy založené na kľúčových slovách, ako napríklad bag-of-words (\gls{bow}), one-hot kódovanie, latentné sémantické indexovanie (\gls{lsindexing}) alebo term frequency-inverse document frequency (\gls{tfidf}). Tieto prístupy sú výpočtovo efektívne a vhodné pre jednoduché dopyty, avšak môžu mať problémy pri komplexných dopytoch, ktoré si vyžadujú porozumenie kontextu~\cite{khan2026retrieval}.

    \item \textbf{Husté (dense) vektorové reprezentácie} využívajú embeddingové \gls{llm} na generovanie embeddingov, ktoré zachytávajú sémantický význam textu. Tento prístup je efektívnejší pre zložité dopyty, vyžaduje si však viac výpočtových zdrojov než riedke prístupy~\cite{khan2026retrieval}.
...
Pre tento výskum sa ako primárny prístup zvolilo husté (dense) vyhľadávanie, keďže je vhodnejšie pre komplexné dopyty a dokáže zachytiť sémantický význam dokumentov, čo je kľúčové pre úlohu výberu vhodných dokumentov na základe používateľských dopytov.
...
Manuálové stránky sa delia do konvenčných skupín a sekcií. Kľúčovými sekciami sú \textbf{DESCRIPTION} (podrobný popis programu, funkčnosti a použitia) a \textbf{OPTIONS} (prepínače, argumenty a ich popisy)~\cite{man_man}.
...
\begin{enumerate}
    \item 1. Klasifikátor príkazov a dopytov ((ktorý bol neskôr zúžený na konceptuálny prototyp)
    \item Vyhľadávač dokumentov, ktorý vytvára a prehľadáva embeddingové indexy, založený na knižnici \gls{faiss} a nástroji Ollama pre lokálne spúšťanie modelov
...
Okrem core komponentov zahŕňa implementácia aj používateľské procesy pre inicializáciu systému, budovanie indexu, vykonávanie dopytov a zmeny konfigurácie.

Komponent klasifikátora je navrhnutý ako prototyp. Môže byť vyvolaný nad aktuálnym používateľským vstupným bufferom a vrátiť binárne rozhodnutie, avšak integrácia tohto riešenia do všeobecného shellu sa ukázala ako nepraktická. Nie je však bez potenciálu, keďže niektoré z existujúcich riešení (napríklad llm-zsh-plugin) ukázali, že existujú špecifické spôsoby aplikácie, ako to dosiahnuť.

Komponent vyhľadávača (retriever) zabezpečuje zber dokumentov, generovanie embeddingov a ukladanie vektorových indexov, pričom podporuje tri prístupy k vyhľadávaniu:
\begin{enumerate}
...
Technologický stack je postavený na jazyku Python s klientskou knižnicou kompatibilnou s OpenAI pre prístup k modelom, knižnicou \gls{faiss} pre vyhľadávanie podobnosti a štandardnými systémovými knižnicami pre správu súborového systému a subprocesov. Ollama poskytuje lokálne runtime prostredie pre modely s konfigurovateľnou dĺžkou kontextu a podporou GPU. Testovacia sada je riadená konfiguračnými súbormi, čo umožňuje reprodukovateľné spúšťanie naprieč variantmi modelov a stratégiami vyhľadávania.
...
\paragraph{Hlavné zistenia}
Vyhľadávanie na úrovni dokumentov a vyhľadávanie na úrovni dokumentov s preusporiadaním pasáží vykazujú najlepší výkon pri použití malých embeddingových modelov, ako je ibm/granite-embedding:30m. Latencia preusporiadania pasáží v reálnom čase je dostatočne nízka na to, aby fungovala lepšie než prístup na úrovni pasáží, pričom zachováva malú veľkosť výstupu a rovnakú presnosť. Naopak, pri použití väčších embeddingových modelov, ako je qwen3-embedding:0.6b, prístup na úrovni pasáží prekonáva ostatné prístupy, keďže výpočtový čas potrebný na tvorbu vektorov pre celý dokument alebo preusporiadanie v reálnom čase sa stáva príliš nákladným.
...
\begin{itemize}
    ...
    \item \textbf{Optimalizácia softvéru:} Zlepšiť cacheovanie, načítavanie modelov a optimalovai vyhľadávania.
...
Táto práca poskytuje empirické dôkazy o tom, ktoré prístupy k lokálnym \gls{llm} sú životaschopné v prostrediach terminálu, identifikuje chybové režimy klasifikácie príkazov a agentového riadenia a ponúka funkčného open-source asistenta založeného na \gls{rag} ako základ pre ďalší výskum. Výsledný systém je užitočný ako offline asistent pre používateľov \gls{cli}, pretože dokáže odpovedať na otázky o lokálnych dokumentoch bez závislosti od siete. Hlavným poznatkom je, že lokálne systémy \gls{llm} sa stávajú efektívnymi vtedy, keď sú vhodne kombinované s optimalizovanými stratégiami vyhľadávania na úrovni pasáží.% \subsection*{Introduction}
